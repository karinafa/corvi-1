...
switch ($_GET["accion"])
{
	case "listar_evento":
	{
		...
		if ($fila=$query->fetch_array())
		{
			do
			{
				echo "<p>".$fila["rut_com"]."".$fila["rolid"]."".$fila["evento"]." ".$fila["hora"]. "<a href='#' class='eliminar_evento' rel='".$fila["id"]."' title='Eliminar este Evento del ".fecha($_GET["fecha"])."'><img src='../assets/img/delete.png'></a></p>";
			}
	...
	case "guardar_evento":
	{
		$isql="insert into ".$tabla." (fecha,evento,hora,rolid,rut_com) values ('".$_GET["fecha"]."','".strip_tags($_GET["evento"])."','".$_GET["hora"]."','".$_GET["rolid"]."','".$_GET["rut_com"]."')";
                //$isql="insert into ".$tabla." (rolid,rut_com,rut_ven) values ('".$_GET["rolid"]."','".strip_tags($_GET["rut_com"])."','".$_GET["rut_ven"]."')";
                $myerr->ErrorFile($isql);
                $query=$db->query($isql);
       
             
                
		if ($query) echo "<p class='ok'>$isql</p>";
		else echo "<p class='error'>Se ha producido un error agendando visita.</p>";
		break;
	}
	case "borrar_evento":
	{
		$query=$db->query("delete from ".$tabla." where id='".$_GET["id"]."' limit 1");
		if ($query) echo "<p class='ok'>Visita eliminada correctamente.</p>";
		else echo "<p class='error'>Se ha producido un error eliminando la visita.</p>";
		break;
	}
	case "generar_calendario":
	{
		...
		/* comprobamos si el año es bisiesto y creamos array de dias */
		...
		$eventos=array();
                //$mysql = "select fecha,count(id) as total from ".$tabla." where month(fecha)='".$fecha_calendario[1]."' and year(fecha)='".$fecha_calendario[0]."' group by fecha";
		echo $mysql;
		...
		echo "<h2>Calendario de Visitas para: ".$meses[intval($fecha_calendario[1])]." de ".$fecha_calendario[0]." <abbr title='S&oacute;lo se pueden agregar eventos en d&iacute;as h&aacute;biles y en fechas futuras (o la fecha actual).'</abbr></h2>";
			...
			for ($i=1;$i<=$diasdespues;$i++)
			{
				if ($tr<$totalfilas)
				{
					if ($i>=$primeromes && $i<=$tope) 
					{
						...
						/* recorremos el array de eventos para mostrar los eventos del dia de hoy */
						if ($hayevento>0) echo "<a href='#' data-evento='#evento".$dia_actual."' class='modal' rel='".$fecha_completa."' title='Hay ".$hayevento." evento'>".$dia."</a>";
						else echo "$dia";
